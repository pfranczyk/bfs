...
import { buildInitArgs, fileExists } from '../smoke-vault.js';
 ...
 *
 * CI flags bypass interactive Inquirer prompts (--no-encryption, --data-shards,
...
export async function suiteB(ctx: SmokeContext): Promise<SuiteResult> {
  ...
  tests.push(
    await runTest('B0', 'setup: directories for CLI init', async () => {
      ...
      await fs.writeFile(
        path.join(cliVaultDir, 'cli-test.txt'),
        'CLI init smoke test',
      );
    }),
  );
  ...
  const ciInitArgs = buildInitArgs(
    ...
    ['--push-mode', 'new_version'],
  );

  tests.push(
    await runTest(
      'B1',
      'bfs init <name> — positional argument + CI flags',
      () => {
  ...
  tests.push(
    await runTest('B1b', '.bfsignore exists after bfs init', async () => {
      assert(
        await fileExists(path.join(cliVaultDir, '.bfsignore')),
        `.bfsignore missing after bfs init`,
      );
    }),
  );
  ...
  tests.push(
    await runTest('B2', '.bfs/config.json exists after init', async () => {
      assert(
        await fileExists(path.join(cliVaultDir, '.bfs', 'config.json')),
        `.bfs/config.json missing after bfs init`,
      );
    }),
  );
  ...
  tests.push(
    await runTest('B5', 'bfs verify after CLI init + push', () => {
      ...
      assert(
        ...
        `expected healthy in: ${out.slice(0, 200)}`,
      );
    }),
  );
  ...
  tests.push(
    await runTest('B7', 'bfs provider add --ci (nowy provider)', async () => {
      ...
      assert(
        out.includes('cli-p4'),
        `expected cli-p4 in output: ${out.slice(0, 200)}`,
      );
    }),
  );
  ...
  tests.push(
    await runTest('B8', 'bfs provider list — cli-p4 visible', () => {
      ...
      assert(
        out.includes('cli-p4'),
        `expected cli-p4 in provider list: ${out.slice(0, 300)}`,
      );
    }),
  );
  ...
  tests.push(
    await runTest('B9', 'bfs provider remove --strategy remove --yes', () => {
      const r = runBfs(
        ['provider', 'remove', 'cli-p4', '--strategy', 'remove', '--yes'],
        cliVaultDir,
      );
      assert(
        r.status === 0,
  ...
  tests.push(
    await runTest('B10', 'bfs provider list — cli-p4 removed', () => {
      ...
      assert(
        !out.includes('cli-p4'),
        `cli-p4 still visible after removal: ${out.slice(0, 300)}`,
      );
    }),
  );
  ...
  const ptDirs = [
    path.join(ctx.sourceDir, 'cli-pt-p1'),
    path.join(ctx.sourceDir, 'cli-pt-p2'),
    path.join(ctx.sourceDir, 'cli-pt-p3'),
  ];

  tests.push(
    await runTest(
      'B11',
      'setup: directories + config files for pass-through',
      async () => {
        await fs.mkdir(ptVaultDir, { recursive: true });
        for (const d of ptDirs) await fs.mkdir(d, { recursive: true });
        await fs.writeFile(
          path.join(ptVaultDir, 'pt-test.txt'),
          'pass-through smoke',
        );
        for (let i = 0; i < ptDirs.length; i++) {
          await fs.writeFile(
            path.join(ptVaultDir, `p${i + 1}.json`),
            JSON.stringify({ path: ptDirs[i] }),
            'utf8',
          );
        }
      },
  ...
  tests.push(
    await runTest(
      'B12',
      'bfs init --ci --provider "local:id --config-file ..." (pass-through)',
      () => {
        const r = runBfs(
          [
            'init',
  ...
  tests.push(
    await runTest(
      'B13',
      'config.json after pass-through init has correct id and path',
      async () => {
        const cfg = JSON.parse(
          await fs.readFile(
            path.join(ptVaultDir, '.bfs', 'config.json'),
            'utf8',
          ),
        ) as {
          providers: Array<{
        ...
        assert(
          cfg.providers.length === 3,
          `expected 3 providers, got ${cfg.providers.length}`,
        );
        assert(
          cfg.providers[0].id === 'pt-p1' && cfg.providers[0].type === 'local',
          `bad provider[0]: ${JSON.stringify(cfg.providers[0])}`,
        );
        assert(
          cfg.providers[0].config.path === ptDirs[0],
          `path mismatch: ${cfg.providers[0].config.path} vs ${ptDirs[0]}`,
        );
      },
    ),
  );
  ...
  tests.push(
    await runTest(
      'B14',
      'bfs init --ci rejects provider name with whitespace',
      () => {
        const r = runBfs(
          [
            'init',
        ...
        assert(
          r.status !== 0,
          `expected non-zero exit for name with space, got ${r.status}`,
        );
        const out = r.stdout + r.stderr;
        assert(
          out.includes('bad name') || /invalid|dozwolone/i.test(out),
          `expected provider name validation error in: ${out.slice(0, 300)}`,
        );
      },
    ),
  );
  ...
  tests.push(
    await runTest(
      'B15',
      'bfs init --ci ftp:nas (no host) shows --provider syntax in error message (EN)',
      () => {
        const r = runBfs(
          [
            '--lang',
        ...
        assert(
          r.status !== 0,
          `expected non-zero exit for missing --host, got ${r.status}`,
        );
        const out = r.stdout + r.stderr;
        assert(
          out.includes('--provider "ftp:nas --host'),
          `expected --provider syntax hint in: ${out.slice(0, 400)}`,
        );
      },
    ),
  );
  ...
  tests.push(
    await runTest(
      'B16',
      'bfs init --ci ftp:nas (no host) shows --provider syntax in error message (PL)',
      () => {
        const r = runBfs(
          [
            '--lang',
        ...
        const out = r.stdout + r.stderr;
        assert(
          out.includes('wewnątrz spec --provider'),
          `expected Polish --provider syntax message in: ${out.slice(0, 400)}`,
        );
      },
    ),
  );
