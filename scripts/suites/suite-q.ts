...
export async function suiteQ(): Promise<SuiteResult> {
  ...
  if (!SSH_ENABLED) {
    tests.push(skipTest('Q1', 'bfs init with 3 SSH providers', SKIP_REASON));
    tests.push(skipTest('Q2', 'bfs push to SSH', SKIP_REASON));
    tests.push(skipTest('Q3', 'bfs pull from SSH', SKIP_REASON));
  ...
  try {
    ...
    tests.push(
      await runTest('Q1', 'bfs init with 3 SSH providers', async () => {
        const args = [
          'init',
          VAULT_NAME,
          '--ci',
    ...
    // Q2 - push to SSH
    tests.push(
      await runTest('Q2', 'bfs push to SSH', async () => {
    ...
    // Q3 - pull from SSH
    tests.push(
      await runTest('Q3', 'bfs pull from SSH', async () => {
