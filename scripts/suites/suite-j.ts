...
export async function suiteJ(): Promise<SuiteResult> {
  ...
  tests.push(
    await runTest(
      'J0',
      'bfs init --ci (without --no-compress) — compression.enabled=true in config',
      async () => {
        originalHashes = await initTestVault(vaultDir, 'zip-vault', [
          { id: 'p1', dir: p1Dir },
          { id: 'p2', dir: p2Dir },
          { id: 'p3', dir: p3Dir },
        ]);
        ...
        assert(
          ...
          `expected compression.enabled=true in config, got: ${JSON.stringify(cfg.compression)}`,
        );
      },
    ),
  );
  ...
  tests.push(
    await runTest(
      'J1',
      'bfs push → exit 0, manifest v1 compressed=true',
      async () => {
        ...
        const manifest = await readJson<{ compressed?: boolean }>(
          path.join(vaultDir, '.bfs', 'manifests', 'v001.json'),
        );
        assert(
          manifest.compressed === true,
          `expected compressed=true in manifest, got: ${JSON.stringify(manifest.compressed)}`,
        );
      },
    ),
  );
  ...
  tests.push(
    await runTest(
      'J2',
      'bfs pull --force → exit 0, SHA-256 of restored files match',
      async () => {
  ...
  tests.push(
    await runTest(
      'J3',
      'bfs init --ci --no-compress → push → manifest.compressed nieobecne',
      async () => {
        ...
        assert(
          rp.status === 0,
          `push exit ${rp.status ?? 'null'}\n${rp.stdout}\n${rp.stderr}`,
        );
        const manifest = await readJson<{ compressed?: boolean }>(
          path.join(noCompDir, '.bfs', 'manifests', 'v001.json'),
        );
        assert(
          manifest.compressed !== true,
          `expected no compressed=true with --no-compress, got: ${JSON.stringify(manifest.compressed)}`,
        );
      },
    ),
  );
  ...
  tests.push(
    await runTest(
      'J4',
      'init (compression ON) + push --no-compress → manifest without compressed',
      async () => {
        const overDir = path.join(tmpBase, 'override-off');
        await initTestVault(overDir, 'ov-vault', [
        ...
        const manifest = await readJson<{ compressed?: boolean }>(
          path.join(overDir, '.bfs', 'manifests', 'v001.json'),
        );
        assert(
          manifest.compressed !== true,
          `expected no compressed=true with push --no-compress, got: ${JSON.stringify(manifest.compressed)}`,
        );
      },
    ),
  );
  ...
  tests.push(
    await runTest(
      'J5',
      'init --no-compress + push --compress → manifest.compressed=true',
      async () => {
        ...
        const manifest = await readJson<{ compressed?: boolean }>(
          path.join(overOnDir, '.bfs', 'manifests', 'v001.json'),
        );
        assert(
          manifest.compressed === true,
          `oczekiwano compressed=true gdy push --compress, got: ${JSON.stringify(manifest.compressed)}`,
        );
      },
    ),
  );
  ...
  tests.push(
    await runTest(
      'J6',
      'bfs push --compress --no-compress → exit != 0, error message',
      () => {
        const r = runBfs(['push', '--compress', '--no-compress'], vaultDir);
        assert(
          r.status !== 0,
          `expected exit != 0 for conflicting --compress + --no-compress, got ${r.status ?? 'null'}`,
        );
        const out = r.stdout + r.stderr;
        assert(
          /compress/i.test(out),
          `expected compression conflict message: ${out.slice(0, 300)}`,
        );
      },
    ),
  );
