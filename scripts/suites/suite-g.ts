...
export async function suiteG(ctx: SmokeContext): Promise<SuiteResult> {
  ...
  tests.push(
    await runTest('G1', 'bfs clear — removes both cache files', async () => {
      ...
      assert(
        out.includes('Cache cleared.') || out.includes('Cache wyczyszczony'),
        `expected clear_done in: ${out.slice(0, 300)}`,
      );
      assert(
        !(await fileExists(pushPending)),
        'push.blob.pending was not removed',
      );
      assert(
        !(await fileExists(pullPending)),
        'pull.blob.pending was not removed',
      );
    }),
  );
  ...
  tests.push(
    await runTest(
      'G2',
      'bfs push --help contains --cache description (EN)',
      () => {
        const r = runBfs(['push', '--help'], gVaultDir, undefined, gEnv);
        const out = r.stdout + r.stderr;
        assert(
          out.includes('Upload cached backup data') ||
            out.includes('cached backup data'),
          `--cache description missing in bfs push --help: ${out.slice(0, 500)}`,
        );
      },
    ),
  );
  ...
  tests.push(
    await runTest(
      'G3',
      'bfs pull --help contains --cache description (EN)',
      () => {
        const r = runBfs(['pull', '--help'], gVaultDir, undefined, gEnv);
        const out = r.stdout + r.stderr;
        assert(
          out.includes('Retry using cached backup data') ||
            out.includes('cached backup data'),
          `--cache description missing in bfs pull --help: ${out.slice(0, 500)}`,
        );
      },
    ),
  );
  ...
  tests.push(
    await runTest(
      'G4',
      'bfs push — abort + push.blob.pending on unreadable file',
      async () => {
        ...
        try {
          ...
          assert(
            r.status === 1,
            `expected exit 1, got ${r.status ?? 'null'}\nstdout: ${r.stdout}\nstderr: ${r.stderr}`,
          );
          const out = r.stdout + r.stderr;
          assert(
            /could not be read|nie można było odczytać/i.test(out),
            `expected push_skipped_header in: ${out.slice(0, 500)}`,
          );
          assert(
            out.includes('bfs push --cache'),
            `expected "bfs push --cache" hint in: ${out.slice(0, 500)}`,
          );
          assert(
            await fileExists(
              path.join(gVaultDir, '.bfs', 'cache', 'push.blob.pending'),
            ),
            'push.blob.pending should exist after abort',
          );
        } finally {
  ...
  tests.push(
    await runTest(
      'G5',
      'bfs push --cache — upload from cache after abort',
      () => {
  ...
  tests.push(
    await runTest(
      'G6',
      'bfs pull -y — abort + pull.blob.pending (EISDIR)',
      async () => {
        ...
        try {
          ...
          const out = r.stdout + r.stderr;
          assert(
            /could not be written|nie można było zapisać/i.test(out),
            `expected pull_skipped_header in: ${out.slice(0, 500)}`,
          );
          assert(
            out.includes('bfs pull --cache'),
            `expected "bfs pull --cache" hint in: ${out.slice(0, 500)}`,
          );
          assert(
            await fileExists(
              path.join(gVaultDir, '.bfs', 'cache', 'pull.blob.pending'),
            ),
            'pull.blob.pending should exist after abort',
          );
        } finally {
  ...
  tests.push(
    await runTest(
      'G7',
      'bfs pull --cache -y — restore file after removing lock',
      async () => {
        ...
        assert(
          await fileExists(normalTxtPath),
          'normal.txt should be restored after pull --cache',
        );
      },
    ),
  );
