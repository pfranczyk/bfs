...
export async function suiteF(ctx: SmokeContext): Promise<SuiteResult> {
  ...
  const langEnv: NodeJS.ProcessEnv = {
    ...process.env,
    XDG_CONFIG_HOME: tmpLangDir,
  };

  tests.push(
    await runTest(
      'F0',
      'setup: isolated language config directory',
      async () => {
  ...
  tests.push(
    await runTest('F1', 'bfs --lang pl → exit 0, Polish output', () => {
      ...
      assert(
        out.includes('Język ustawiony na: pl'),
        `expected Polish confirmation in: ${out.slice(0, 300)}`,
      );
    }),
  );
  ...
  tests.push(
    await runTest('F2', 'settings.json contains language: "pl"', async () => {
      const settings = await readJson<{ language: string }>(
        path.join(tmpLangDir, 'bfs', 'settings.json'),
      );
      assert(
        settings.language === 'pl',
        `oczekiwano language "pl", got: ${JSON.stringify(settings)}`,
      );
    }),
  );
  ...
  tests.push(
    await runTest('F3', 'bfs status (no --lang) → Polish output', () => {
      ...
      const out = r.stdout + r.stderr;
      assert(
        out.includes('Status kopii zapasowej'),
        `expected Polish "Status kopii zapasowej" in: ${out.slice(0, 300)}`,
      );
    }),
  );
  ...
  tests.push(
    await runTest('F4', 'bfs --lang en → exit 0, English output', () => {
      ...
      const out = r.stdout + r.stderr;
      assert(
        out.includes('Language set to: en'),
        `expected English confirmation in: ${out.slice(0, 300)}`,
      );
    }),
  );
  ...
  tests.push(
    await runTest('F5', 'bfs status (no --lang) → English output', () => {
      ...
      const out = r.stdout + r.stderr;
      assert(
        out.includes('Backup status'),
        `expected English "Backup status" in: ${out.slice(0, 300)}`,
      );
    }),
  );
