...
export async function suiteM(): Promise<SuiteResult> {
  ...
  try {
    ...
    tests.push(
      await runTest('M1', 'bfs recovery --provider local --bootstrap "--path …" odbudowuje .bfs/', async () => {
    ...
    tests.push(
      await runTest('M2', 'bfs pull after recovery restores files (SHA match)', async () => {
        ...
        await verifyShaHashes(restoreDir, originalHashes, 'po recovery + pull');
      }),
    );
    ...
    tests.push(
      await runTest('M3', 'bfs recovery --bootstrap bez --provider → exit ≠ 0', async () => {
    ...
    tests.push(
      await runTest('M4', 'bfs recovery --bootstrap bez --name → exit ≠ 0', async () => {
    ...
    tests.push(
      await runTest('M7', 'bfs recovery --bootstrap "--config-file …" odbudowuje .bfs/', async () => {
