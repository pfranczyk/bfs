...
export async function suiteN(ctx: SmokeContext): Promise<SuiteResult> {
  ...
  const pushBlobPending = path.join(
    nVaultDir,
    '.bfs',
    'cache',
    'push.blob.pending',
  );
  const pullBlobPending = path.join(
    nVaultDir,
    '.bfs',
    'cache',
    'pull.blob.pending',
  ...
  tests.push(
    await runTest('N0', 'setup: vault N (partial-commit)', async () => {
      ...
      await fs.writeFile(
        path.join(nVaultDir, 'hello.txt'),
        'Hello, partial commit!',
      );
  ...
  tests.push(
    await runTest(
      'N1',
      'bfs push happy — healthy + no leftover lock/cache',
      async () => {
        ...
        assert(
          /healthy.*3 of 3 uploaded|zdrowa.*3 z 3 wgranych/i.test(out),
          `expected healthy completion: ${out.slice(0, 400)}`,
        );
        assert(
          !(await fileExists(pushLockFile)),
          'push.lock should be removed on healthy push',
        );
        assert(
          !(await fileExists(pushBlobPending)),
          'push.blob.pending should be removed on healthy push',
        );
      },
    ),
  );
  ...
  tests.push(
    await runTest(
      'N2',
      'bfs push partial — exit != 0, degraded manifest, lock retained',
      async () => {
        ...
        try {
          ...
          assert(
            r.status !== 0,
            `expected non-zero exit for degraded push, got ${r.status}`,
          );
          const out = r.stdout + r.stderr;
          assert(
            /degraded|zdegradowan/i.test(out),
            `expected degraded message: ${out.slice(0, 400)}`,
          );
          assert(
            /2 of 3 uploaded|2 z 3 wgranych/i.test(out),
            `expected "2 of 3 uploaded": ${out.slice(0, 400)}`,
          );
          ...
          assert(
            manifest.shards.length === 2,
            `expected manifest.shards.length=2, got ${manifest.shards.length}`,
          );
          assert(
            manifest.health === 'degraded',
            `expected manifest.health=degraded, got ${manifest.health}`,
          );

          assert(
            await fileExists(pushLockFile),
            'push.lock should be retained after degraded push',
          );
          ...
          assert(
            lock.failed.length === 1,
            `expected lock.failed.length=1, got ${lock.failed.length}`,
          );
          assert(
            lock.uploaded.length === 2,
            `expected lock.uploaded.length=2, got ${lock.uploaded.length}`,
          );
          assert(
            await fileExists(pushBlobPending),
            'push.blob.pending should be retained for --cache retry',
          );
        } finally {
  ...
  tests.push(
    await runTest(
      'N3',
      'bfs clear — removes push.lock + repair.lock + both blob.pending',
      async () => {
        ...
        assert(
          !(await fileExists(pushBlobPending)),
          'push.blob.pending not removed',
        );
        assert(
          !(await fileExists(pullBlobPending)),
          'pull.blob.pending not removed',
        );
      },
    ),
  );
  ...
  tests.push(
    await runTest(
      'N4',
      'bfs push --cache without push.lock — exit != 0, missing files reported',
      async () => {
        ...
        const out = r.stdout + r.stderr;
        assert(
          /missing|brakuje/i.test(out),
          `expected "missing" hint: ${out.slice(0, 400)}`,
        );
      },
    ),
  );
  ...
  tests.push(
    await runTest(
      'N5',
      'bfs status — push-disabled warn when scheme is 3/0',
      async () => {
        ...
        assert(
          r.status === 0,
          `expected exit 0 for status, got ${r.status}\n${r.stderr}`,
        );
        const out = r.stdout + r.stderr;
        assert(
          /push disabled|push wyłączony/i.test(out),
          `expected push-disabled warn: ${out.slice(0, 400)}`,
        );
        assert(
          /3\/0/.test(out),
          `expected "3/0" in output: ${out.slice(0, 400)}`,
        );
      },
    ),
  );
  ...
  const nCwdPushBlobPending = path.join(
    nCwdVaultDir,
    '.bfs',
    'cache',
    'push.blob.pending',
  ...
  tests.push(
    await runTest(
      'N6',
      'bfs --cwd <vault> push — healthy, cache + lock land under --cwd',
      async () => {
        ...
        await fs.writeFile(
          path.join(nCwdVaultDir, 'hello.txt'),
          'Hello from --cwd!',
        );
        ...
        const providers: ProviderConfig[] = [
          {
            ...
            type: 'local',
            adapterPackage: null,
            config: { path: nCwdP1Dir },
          },
          {
            id: 'cwd-p2',
            type: 'local',
            adapterPackage: null,
            config: { path: nCwdP2Dir },
          },
          {
            id: 'cwd-p3',
            type: 'local',
            adapterPackage: null,
            config: { path: nCwdP3Dir },
          },
        ];
        await init(nCwdVaultDir, {
          vault_name: 'n-cwd-vault',
          scheme: { data_shards: 2, parity_shards: 1 },
          encryption: {
            enabled: false,
            algorithm: 'aes-256-gcm',
        ...
        assert(
          !(await fileExists(nCwdPushLock)),
          'push.lock should be removed on healthy push under --cwd',
        );
        assert(
          !(await fileExists(nCwdPushBlobPending)),
          'push.blob.pending should be removed on healthy push under --cwd',
        );
        // Manifest v1 must land in the --cwd vault — proves rootDir
        // propagated all the way through manifest write.
        assert(
          await fileExists(
            path.join(nCwdVaultDir, '.bfs', 'manifests', 'v001.json'),
          ),
          'manifest v001.json missing under --cwd vault',
        );
      },
    ),
  );
  ...
  tests.push(
    await runTest(
      'N7',
      'bfs --cwd <vault> push --cache — resume reads cache from --cwd, not process.cwd()',
      async () => {
        ...
        try {
          ...
          assert(
            partial.status !== 0,
            `expected non-zero exit for degraded push, got ${partial.status}`,
          );
          assert(
            await fileExists(nCwdPushLock),
            'push.lock should be retained after degraded push under --cwd',
          );
          assert(
            await fileExists(nCwdPushBlobPending),
            'push.blob.pending should be retained for --cache retry under --cwd',
          );
          ...
          assert(
            resume.status === 0,
            `expected exit 0 on resume, got ${resume.status}\nstdout: ${resume.stdout}\nstderr: ${resume.stderr}`,
          );
          assert(
            !(await fileExists(nCwdPushLock)),
            'push.lock should be cleared after successful --cache --overwrite resume',
          );
          assert(
            !(await fileExists(nCwdPushBlobPending)),
            'push.blob.pending should be cleared after successful resume',
          );
        } finally {
          await fs.unlink(nCwdP3Dir).catch(() => {});
          await fs.mkdir(nCwdP3Dir, { recursive: true });
