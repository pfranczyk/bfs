...
export async function suiteI(ctx: SmokeContext): Promise<SuiteResult> {
  ...
  tests.push(
    await runTest('I1', 'prune without arguments + closed stdin → clean exit', () => {
      ...
      assert(!out.includes('User force closed'), `stderr must not contain "User force closed": ${out.slice(0, 400)}`);
    }),
  );

  tests.push(
    await runTest('I2', 'prune --keep-last 999 --yes → exit 0 (nothing to remove)', () => {
      const r = runBfs(['prune', '--keep-last', '999', '--yes'], ctx.vaultDir);
