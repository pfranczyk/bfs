...
export async function suiteC(ctx: SmokeContext): Promise<SuiteResult> {
  ...
  tests.push(
    await runTest('C2', 'shards na dysku (v1)', async () => {
      for (const provDir of [ctx.provider1Dir, ctx.provider2Dir, ctx.provider3Dir]) {
        ...
        assert(shards.length === 1, `${provDir}: expected 1 shard v1, got ${shards.length} (files: ${files.join(', ')})`);
      }
    }),
  );
  ...
  tests.push(
    await runTest('C3', 'bfs status', () => {
      ...
      assert(/v1|version.?1/i.test(out), `expected v1 in output: ${out.slice(0, 300)}`);
    }),
  );
  ...
  tests.push(
    await runTest('C5', 'bfs verify', () => {
      ...
      assert(r.status === 0, `exit ${r.status ?? 'null'}\n${r.stderr}`);
      const out = r.stdout + r.stderr;
      assert(/healthy|zdrow/i.test(out), `expected healthy/zdrowa in output: ${out.slice(0, 300)}`);
    }),
  );
  ...
  tests.push(
    await runTest('C6', 'bfs provider list', () => {
      ...
      assert(r.status === 0, `exit ${r.status ?? 'null'}\n${r.stderr}`);
      const out = r.stdout + r.stderr;
      assert(out.includes('p1') && out.includes('p2') && out.includes('p3'), `expected provider names p1/p2/p3 in output: ${out.slice(0, 300)}`);
    }),
  );
