...
# repair --restore-headers, MISSING sidecar (local, --no-enc): after a relocate
# writes an hdr_ sidecar next to EVERY shard, one sibling's sidecar is DELETED.
# `bfs verify` surfaces the --restore-headers advisory; `bfs repair
# --restore-headers` rebuilds the missing sidecar from the CURRENT config map +
# the in-shard frozen fields (no payload pull), so verify's advisory disappears
# and a fresh recovery from the rebuilt sibling restores the files byte-for-byte.

SCENARIO_NAME="repair --restore-headers rebuilds a missing sidecar (local, no-enc)"
SCENARIO_DESC="3L 2/1; relocate p0 writes sidecars, delete p1's hdr_, repair --restore-headers restores it, advisory gone, recover from p1 byte-for-byte"
REQUIRES_LOCAL=3
REQUIRES_FTP=0

scenario_run() {
  local vault="$SC_DIR/vault" base="$SC_DIR/baseline.txt" name="bfs76"
  ...
  # FIX (RED today): rebuild every missing sidecar for all versions from config.
  ...
  if printf '%s' "$BFS_OUT" | grep -qF -- "--restore-headers"; then
    _fail "verify still advises --restore-headers after repair rebuilt the sidecar"
  fi
