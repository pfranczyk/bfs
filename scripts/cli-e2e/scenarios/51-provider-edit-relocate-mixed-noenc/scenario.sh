...
# `bfs provider edit` on a mixed pool (local + FTP), UNENCRYPTED backup
# (`--no-enc`). Same relocation as the encrypted variant: physically move both a
# local and an FTP provider's storage, repoint the providers with
# `bfs provider edit`, then verify + restore + round-trip a second version. The
# unencrypted path exercises a plaintext location map and a password-less
# push/pull, confirming edit is independent of the encryption mode.

SCENARIO_NAME="provider edit: relocate local+FTP (no-enc)"
SCENARIO_DESC="2L+1F 2/1 unencrypted; edit local + FTP --path to moved storage; verify+pull+push"
REQUIRES_LOCAL=2
REQUIRES_FTP=1

scenario_run() {
  local vault="$SC_DIR/vault" base="$SC_DIR/baseline.txt" name="bfs51"
  ...
  assert_manifest_absent "$vault" 1 '"encrypted": true'
