#!/bin/sh
# Run inside Docker: installs bfs-vault@0.3.0 and creates a backup.
# Provider format: type:id:path  (colon-separated, no space)
# No compression support in this version. Streaming pipeline (shard format V2).
set -e
BASE="$1"
VAULT_NAME="$2"

npm install -g bfs-vault@0.3.0
