...
describe('init', () => {
  ...
  it('CI: should abort when --ci given without vault_name argument', async () => {
    ...
    expect(
      capture.errors.some((e) =>
        e.includes('--ci mode requires backup name as positional argument'),
      ),
    ).toBe(true);
  });
  ...
  it('CI: should abort when --data-shards is missing', async () => {
    ...
    expect(
      capture.errors.some((e) =>
        e.includes('--ci mode requires --data-shards and --parity-shards'),
      ),
    ).toBe(true);
  });
  ...
  it('CI: should abort when --data-shards < 2', async () => {
    ...
    expect(
      capture.errors.some((e) =>
        e.includes('--data-shards must be an integer >= 2'),
      ),
    ).toBe(true);
  });
  ...
  it('CI: should abort when --parity-shards < 1', async () => {
    ...
    expect(
      capture.errors.some((e) =>
        e.includes('--parity-shards must be an integer >= 1'),
      ),
    ).toBe(true);
  });
  ...
  it('CI: should abort when --provider count does not match N+K', async () => {
    ...
    expect(result).toBe('abort');
    expect(mockInit).not.toHaveBeenCalled();
    expect(capture.errors.some((e) => e.includes('--ci mode requires 3'))).toBe(
      true,
    );
  });
  ...
  describe('CI: --provider pass-through grammar', () => {
    ...
    it('should forward every post-head token as rawArgs (no BFS interpretation)', async () => {
      ...
      expect(thirdArg?.rawArgs).toEqual([
        '--bucket',
        'mine',
        '--region',
        'us-east-1',
    ...
    it('should preserve quoted values containing spaces in rawArgs', async () => {
      ...
      const [firstArg] = spy.mock.calls[0] ?? [];
      expect(firstArg?.rawArgs).toEqual([
        '--label',
        'my first disk',
        '--config-file',
        './configs/d1.json',
    ...
    it('should construct the provider io with workDir=rootDir', async () => {
      const createCliProviderIOSpy = vi.mocked(
        await import('../../src/providers/provider.js'),
      ).createCliProviderIO;
