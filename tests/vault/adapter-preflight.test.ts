...
import {
  ...
  missingAdapterError,
} from '../../src/vault/adapter-preflight.js';
...
function fakeProvider(
  id: string,
  pkgSpec: string | null,
  type: string = FAKE_TYPE,
): ProviderConfig {
  return {
    id,
    type,
    adapterPackage: pkgSpec,
    config: {},
  };
}
...
describe('detectMissingAdapters', () => {
  it('should return empty when every type is registered', () => {
    const result = detectMissingAdapters([
      localProvider('p1'),
      localProvider('p2'),
    ]);
  ...
  it('should report one MissingAdapter per unique unregistered type', () => {
    const result = detectMissingAdapters([
      fakeProvider('p1', 'bfs-adapter-x@1.0.0'),
      fakeProvider('p2', 'bfs-adapter-x@1.0.0'),
    ]);
  ...
  it('should distinguish built-in (adapterPackage=null) from external', () => {
    const result = detectMissingAdapters([
      fakeProvider('bin', null, 'not-real-built-in'),
      fakeProvider('ext', 'bfs-adapter-x@2.0.0'),
    ]);
...
describe('formatMissingAdaptersMessage', () => {
  it('should render one install line per missing external adapter', () => {
    const msg = formatMissingAdaptersMessage([
      {
        type: 'ssh',
        adapterPackage: 'bfs-adapter-ssh@1.0.1',
        providerIds: ['ssh-1'],
      },
      {
        type: 'cloud',
        adapterPackage: '@corp/bfs-adapter-cloud@2.3.0',
        providerIds: ['c1'],
      },
    ]);
  ...
  it('should skip built-in missing entries (separate error path)', () => {
    const msg = formatMissingAdaptersMessage([
      { type: 'broken-builtin', adapterPackage: null, providerIds: ['x'] },
    ]);
    expect(msg).toBe('');
  });
...
describe('checkVersionMismatch', () => {
  ...
  afterEach(() => {
    ...
    (
      providerRegistry as unknown as { entries: Map<string, unknown> }
    ).entries.delete(FAKE_META_TYPE);
  });
    ...
    const result = checkVersionMismatch([
      {
        id: 'ok',
        type: FAKE_META_TYPE,
        adapterPackage: 'bfs-adapter-fake@1.2.3',
        config: {},
      },
    ]);
    expect(result).toEqual([]);
    ...
    const result = checkVersionMismatch([
      {
        id: 'upgraded',
        type: FAKE_META_TYPE,
        adapterPackage: 'bfs-adapter-fake@0.9.0',
        config: {},
      },
    ]);
    expect(result).toHaveLength(1);
    ...
    const result = checkVersionMismatch([
      {
        id: 'patched',
        type: FAKE_META_TYPE,
        adapterPackage: 'bfs-adapter-fake@1.2.1',
        config: {},
      },
    ]);
    expect(result).toHaveLength(1);
