...
describe('resolveSafeChildPath', () => {
  ...
  it('should reject a leading ".." segment', () => {
    expect(() => resolveSafeChildPath(root, '../evil')).toThrow(
      UnsafePathError,
    );
  });

  it('should reject a ".." segment buried mid-path', () => {
    expect(() => resolveSafeChildPath(root, 'a/../../evil')).toThrow(
      UnsafePathError,
    );
  });
  ...
  it('should reject an absolute POSIX path', () => {
    expect(() => resolveSafeChildPath(root, '/etc/passwd')).toThrow(
      UnsafePathError,
    );
  });
  ...
  it('should reject an absolute Windows path', () => {
    expect(() =>
      resolveSafeChildPath(root, 'C:\\Windows\\system32\\drivers\\etc\\hosts'),
    ).toThrow(UnsafePathError);
  });

  it('should reject a UNC path', () => {
    expect(() => resolveSafeChildPath(root, '\\\\server\\share\\evil')).toThrow(
      UnsafePathError,
    );
  });
