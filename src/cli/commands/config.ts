...
export function registerConfig(program: Command): void {
  program
    ...
    .action(async (opts: { cacheDir?: string | true; tempDir?: string | true; maxRam?: string | true; reset?: boolean; on?: string; off?: string }, cmd: Command) => {
      ...
      if (opts.on !== undefined || opts.off !== undefined) {
        ...
        if (featureKey === 'compression') {
          if (!config.compression) config.compression = { enabled: enable, algorithm: 'deflate' };
          else config.compression.enabled = enable;
        } else {
          if (!config.encryption) config.encryption = { enabled: enable, algorithm: 'aes-256-gcm', kdf: 'argon2id' };
          else config.encryption.enabled = enable;
        }
        await writeConfig(rootDir, config);
        const displayName = featureKey === 'compression' ? t('config_label_compression') : t('config_label_encryption');
        success(fmt(enable ? 'config_feature_on' : 'config_feature_off', displayName));
        info(t('config_next_push'));
        return;
      }

      if (opts.reset === true && opts.cacheDir === undefined && opts.tempDir === undefined && opts.maxRam === undefined) {
        error(t('config_reset_no_field'));
        return;
      }

      const hasChange = opts.cacheDir !== undefined || opts.tempDir !== undefined || opts.maxRam !== undefined;

      if (!hasChange) {
        // Display current settings
        const cacheDefault = path.join(rootDir, '.bfs', 'cache');
        info(t('config_current_settings'));
        info(`  cache-dir: ${config.cache_dir ?? fmt('config_cache_default', cacheDefault)}`);
        info(`  temp-dir:  ${config.temp_dir ?? t('config_temp_default')}`);
        info(`  max-ram:   ${config.max_ram_mb != null ? `${config.max_ram_mb} MB` : t('config_ram_auto')}`);
        const compressState = config.compression?.enabled ? t('status_enc_enabled') : t('status_enc_disabled');
        const encState = config.encryption?.enabled ? t('status_enc_enabled') : t('status_enc_disabled');
        info(`  ${t('config_label_compression').padEnd(14)} ${compressState}`);
      ...
      if (opts.maxRam !== undefined) {
        config.max_ram_mb = isReset(opts.maxRam) ? null : parseInt(opts.maxRam as string, 10);
        changed = true;
      }

      if (changed) {
        await writeConfig(rootDir, config);
        success(opts.reset === true || opts.cacheDir === true || opts.tempDir === true || opts.maxRam === true ? t('config_reset') : t('config_updated'));
      }
    });
}
